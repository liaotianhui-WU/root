...
void grow(Domain &dom, double dx){
    Particle *p = dom.GetParticle(2);
    Quaternion_t q = p->Q;
    Vec3_t elongation;
    Rotation(Vec3_t(0, 0, -dx), q, elongation);
    p -> Verts[1]=Verts[1] + elongation;
    p -> Edges[0]= Edge(p -> Verts[0], p -> Verts[1])
}
...
int main(int argc, char **argv) try
{
    ...
    dom.AddRice(/*Tag*/2, /*position*/Vec3_t(0.0,0.0,1.0), Rroot, /*L*/0.0, rho_root, /*angle*/0.0, /*axis*/OrthoSys::e2);
    ...
    dom.Initialize(dt);
    dom.Solve(tf, dt, dtOut, grow(dom, dx), NULL, "rice");
}
MECHSYS_CATCH